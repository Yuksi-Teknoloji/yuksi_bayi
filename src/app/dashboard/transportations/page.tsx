...
const STATUS_BUTTONS: {
  ...
  {
    key: "kurye_cagrildi",
    label: "Kurye Çağrıldı",
    className: "bg-orange-500 hover:bg-orange-600",
  },
  {
    key: "kuryeye_verildi",
    label: "Kuryeye Verildi",
...
export default function DealerTransportationsPage() {
  ...
  /* ---- orders ---- */
  const [status, setStatus] = React.useState<string>("hazirlaniyor"); // aktif durum filtresi
  ...
  const loadOrders = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/yuksi/dealer/restaurants/${restaurantId}/order-history?` +
          params.toString(),
      ...
      const list: OrderRow[] = Array.isArray(data?.orders)
        ? data!.orders.map(
            (o: any): OrderRow => ({
              ...
              dropoff_lat:
                o?.dropoff_lat == null ? null : Number(o.dropoff_lat),
              dropoff_lng:
                o?.dropoff_lng == null ? null : Number(o.dropoff_lng),
              type: String(o?.type ?? ""),
  ...
  React.useEffect(() => {
    if (rows.length)
      setSelectedOrderId((prev) =>
        prev && rows.find((r) => r.id === prev) ? prev : rows[0].id
      );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-600">
            Restoran paketlerini harita üzerinden anlık olarak izle ve durumlara
            göre filtrele.
          </p>
        </div>
      ...
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between my-3">
          ...
          <div className="text-xs text-neutral-500">
            Toplam Sipariş: <b className="tabular-nums">{totalCount}</b> ·
            Toplam Tutar:{" "}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-4">
                ...
                <>
                  <div className="pointer-events-auto absolute right-3 top-3 z-10 flex flex-col gap-2">
                    ...
                    <button
                      ...
                      title="Düzenle"
                      aria-label="Düzenle"
                      className="grid cursor-pointer h-10 w-10 place-items-center rounded-md bg-amber-300 text-white shadow"
                    >
                  ...
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 w-full px-4">
                    {/* Status buttons */}
                    <div className="grid grid-cols-3 mx-auto w-full gap-3 px-3">
                      {STATUS_BUTTONS.map((b) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {count > 0 && (
                              <span className="ml-auto inline-flex h-5 min-w-[20px] items-center justify-center rounded-full bg-white/30 px-2 font-mono font-semibold tabular-nums">
                                {count}
                              </span>
                            )}
                          </button>
          ...
          <aside className="flex flex-col gap-3 rounded-2xl border border-neutral-200 bg-neutral-50 p-4">
            {selectedOrder ? (
              <>
      ...
      <section className="mt-4 border-t border-neutral-200 bg-white rounded-2xl p-3">
        {/* Bottom order list */}
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={[
                      ...
                      .join(" ")}
                  />
                  <div className="font-semibold text-xs">#{o.code}</div>
                </div>
